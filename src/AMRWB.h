...
public:
    ...
    /**
     * @brief Construct a new AMRWB codec
     * @param enableDTX Enable discontinuous transmission (DTX)
     */
    inline AMRWB(bool enableDTX = false) {
        // Initialize the encoder and decoder
        encoderState = E_IF_init();
        decoderState = D_IF_init();
    ...
    inline int encode(const int16_t* pcmSamples, size_t sampleCount, 
        ...
        for (size_t i = 0; i < sampleCount && bytesWritten < amrBufferSize; i += samplesPerFrame) {
     ...
     */
    inline int decode(const uint8_t* amrData, size_t amrSize, 
                     int16_t* pcmSamples, size_t maxSampleCount) override {
        if (!isInitialized || !amrData || amrSize == 0 || !pcmSamples || maxSampleCount == 0) {
            return 0;
        }
        ...
        while (offset < amrSize && totalSamplesDecoded + samplesPerFrame <= maxSampleCount) {
            ...
            switch (frameType) {
                case 0: frameSize = 18; break;  // 6.60 kbit/s
                case 1: frameSize = 24; break;  // 8.85 kbit/s
                case 2: frameSize = 33; break;  // 12.65 kbit/s
                case 3: frameSize = 37; break;  // 14.25 kbit/s
                case 4: frameSize = 41; break;  // 15.85 kbit/s
                case 5: frameSize = 47; break;  // 18.25 kbit/s
                case 6: frameSize = 51; break;  // 19.85 kbit/s
                case 7: frameSize = 59; break;  // 23.05 kbit/s
                case 8: frameSize = 61; break;  // 23.85 kbit/s
                default: frameSize = 1; break;  // SID, etc.
            }
